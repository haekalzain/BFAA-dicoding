...
import com.example.tes.githubusersubmission2.Utility
import com.example.tes.githubusersubmission2.Utility.Constants.EXTRA_DATA
import com.example.tes.githubusersubmission2.adapter.ViewPagerAdapter
import com.example.tes.githubusersubmission2.model.UserItems
import com.example.tes.githubusersubmission2.viewModel.DetailViewModel
...
class DetailUsersActivity : AppCompatActivity() {
    ...
    private fun configDetailViewModel(getUsername: String) {
        ...
        detailViewModel.getDetailUser().observe(this, Observer {
            ...
            nameUser.text = Utility.getData(it.name)
            companyUser.text = Utility.getData(it.company)
            locationUser.text = Utility.getData(it.location)
            repositoryUser.text = Utility.getData(it.repository.toString())
            followersUser.text = resources.getString(
                R.string.followers,
               it.followers
            )
